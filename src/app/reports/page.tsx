...
function StatCard({ label, value, icon, color }: { label: string; value: string | number; icon: string; color: string }) {
  ...
  return (
    <div className="glass-card p-4 sm:p-6 rounded-xl hover:shadow-[0_0_20px_0_rgba(107,251,154,0.15)] transition-shadow">
      ...
      <h3 className="text-xl sm:text-2xl lg:text-3xl font-bold font-[Montserrat] text-green-400 mt-1 truncate">{value}</h3>
    </div>
  );
}
...
export default function ReportsPage() {
  ...
  useEffect(() => {
    getReports().then(setReport).catch(() => setReport(null)).finally(() => setLoading(false));
  }, []);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl sm:text-2xl font-semibold font-[Montserrat] text-gray-200">Reports & Analytics</h2>
        <p className="text-gray-400 text-xs sm:text-sm mt-1">Performance overview of billiard & cafe operations</p>
      </div>
      ...
      <div className="glass-card rounded-xl p-4 sm:p-6">
        <h4 className="text-base sm:text-lg font-semibold font-[Montserrat] text-gray-200 mb-3 sm:mb-4">Best Selling Products</h4>
        {report?.best_sellers && report.best_sellers.length > 0 ? (
          <div className="space-y-3">
            {report.best_sellers.map((item: any, idx: number) => (
              <div key={item.name} className="flex items-center gap-3 sm:gap-4 p-3 rounded-lg bg-gray-900 border border-gray-800 hover:bg-gray-800/50 transition-colors">
                <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center font-bold text-xs sm:text-sm flex-shrink-0 ${
                  idx === 0 ? 'bg-yellow-400/20 text-yellow-400' : 'bg-gray-700 text-gray-300'
                }`}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] sm:text-xs text-gray-400">{item.quantity} items sold</p>
                </div>
                <span className="text-xs sm:text-sm font-bold text-green-400">{item.quantity}x</span>
              </div>
            ))}
          </div>
        ) : (
          <div className="text-center py-10 sm:py-12 text-gray-400 text-xs sm:text-sm">No sales data yet.</div>
        )}
      </div>
    </div>
  );
